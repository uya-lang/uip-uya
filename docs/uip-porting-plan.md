...
- RST 关闭
- periodic 下最小重传与超时关闭
- `SynSent` / `SynRcvd` / `LastAck` 的最小 retransmit 路径
- 多个 TCP 状态下的 ACK/SEQ 合法性与异常输入丢弃分支
...
- 其他当前未覆盖的 TCP corner case

### IPv4 / ICMP / UDP

- 除 echo reply 外的其他 ICMP 类型
- 更完整的 IPv4 边界分支
- 原版 UDP 其余边界行为
- `UIP_REASSEMBLY` 当前仅锁定现状：MF=1 且 offset=0 的 IPv4 首片可能继续进入常规 TCP/UDP/ICMP 路径，offset!=0 的片段当前不会按普通 TCP/UDP conn 或 listener 匹配，two-fragment UDP 当前不会完成为普通 UDP conn 投递
...
如后续要继续贴近 C 版应用协作方式，还需确认并补齐是否需要提供：
